...
namespace Yarn.Unity
{
    ...
    internal static class DiagnosticUtility
    {
        ...
        {
            var result = count == 1 ? name : name + "s";
    ...
    {
        internal static HashSet<ActionRegistrationMethod> ActionRegistrationMethods = new();
        ...
        private static Converter[] CreateConverters(MethodInfo method)
        {
            ...
            var result = new Converter[parameterInfos.Length];
        ...
        private static Converter CreateConverterFunction(Type targetType, string parameterName)
        {
            ...
            // find the GameObject.
            if (typeof(GameObject).IsAssignableFrom(targetType)) return (arg, i) => GameObject.Find(arg);
            ...
            if (typeof(bool).IsAssignableFrom(targetType))
                return (arg, i) =>
                {
                    ...
                    if (arg.Equals(parameterName, StringComparison.InvariantCultureIgnoreCase)) return true;

                    // If the argument can be parsed as boolean true or false,
                    // return that result.
                    if (bool.TryParse(arg, out var res)) return res;
        ...
        internal class CommandRegistration : ICommand
        {
            public enum CommandType
            {
                ...
                /// </summary>
                /// <remarks>
                ///     Code that invokes this command should use
                ///     <see
                ///         cref="MonoBehaviour.StartCoroutine(IEnumerator)" />
                ///     to begin
                ///     the coroutine.
                /// </remarks>
                IsCoroutine,
                ...
                ///     return <see cref="void" />, <see cref="Coroutine" />, or
                ///     <see
                ///         cref="IEnumerator" />
                ///     .)
                /// </summary>
            ...
            private readonly Dictionary<Type, string> TypeFriendlyNames = new()
            {
                { typeof(int), "number" },
                { typeof(float), "number" },
            ...
            public CommandType Type
            {
                get
                {
                    ...
                    if (typeof(void).IsAssignableFrom(returnType)) return CommandType.IsVoid;
                    if (typeof(IEnumerator).IsAssignableFrom(returnType)) return CommandType.IsCoroutine;
                    if (typeof(Coroutine).IsAssignableFrom(returnType)) return CommandType.ReturnsCoroutine;
            ...
            private (int Min, int Max) ParameterCount
            {
                get
                {
                    ...
                    var optional = 0;
                    var lastCommandIsParams = false;
                    foreach (var parameter in parameters)
                    {
                        if (parameter.IsOptional) optional += 1;
                    ...
                    if (lastCommandIsParams) max = int.MaxValue;
                    return (min, max);
                }
            }
            ...
            public string UsageString
            {
                get
                {
                    ...
                    foreach (var parameter in Method.GetParameters())
                    {
                        ...
                        if (!TypeFriendlyNames.TryGetValue(type, out typeName)) typeName = type.Name;

                        var displayName = $"{parameter.Name} <i>({typeName})</i>";

                        if (parameter.IsOptional) displayName = $"[{displayName} = {parameter.DefaultValue}]";
                ...
                out string? message)
            {
                ...
                var lastParameterIsArray =
                    parameters.Length > 0 && parameters[parameters.Length - 1].ParameterType.IsArray;
                ...
                if (argumentCount < min || (argumentCount > max && !lastParameterIsArray))
                {
                    ...
                    if (min == 0)
                        ...
                        requirementDescription =
                            $"{min} {DiagnosticUtility.EnglishPluraliseNounCount(max, "parameter")}";
                    message =
                ...
                for (var i = 0; i < argumentCount; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (converter == null)
                            ...
                            {
                                message =
                                    $"Can't convert parameter {i} to {parameters[i].ParameterType.Name}: {e.Message}";
                                result = default;
                                return CommandDispatchResult.ParameterParseStatusType.InvalidParameterType;
                            }
                    }
            ...
            internal CommandDispatchResult Invoke(MonoBehaviour dispatcher, List<string> parameters)
            {
                ...
                var parseArgsStatus = TryParseArgs(parameters.ToArray(), out var finalParameters, out var errorMessage);
                ...
                if (returnValue is YarnTask yarnTask)
                    return new CommandDispatchResult(
                        CommandDispatchResult.StatusType.Succeeded,
                        yarnTask
                    );
